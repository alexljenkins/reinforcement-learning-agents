...
class Agent():
    ...
    def get_exploration_rate(self):
        return self.strat_vals[1] + (self.strat_vals[0] - self.strat_vals[1]) * math.exp(-1.0 * self.current_step * self.strat_vals[2])
...
def visualize_processed_vs_unprocessed_image():
    ...
    sim.em.reset()
    screen = sim.em.render('rgb_array')
    ...
    screen = sim.em.get_processed_screen()
    plt.figure()
...
class Simulation():
    ...
    def initialize_static_values(self):
        ...
        self.epochs = 200 # epochs before saving and quitting
    ...
    def save_trained_model(self, path):
        torch.save({
                ...
                'epoch': len(self.epoch_durations),
...
if __name__ == '__main__':
    ...
    for epoch in range(sim.epoch, sim.epoch+sim.epochs):
        # a complete epoch of agent training
        sim.sim_epoch()
